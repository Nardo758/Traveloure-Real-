...
--
-- Idempotent: uses IF NOT EXISTS guards so this can be applied to a DB
-- where db:push already created the objects (re-running on dev) or to
-- prod for the first time without inspection.
...
ALTER TABLE "provider_services" ADD COLUMN IF NOT EXISTS "neighborhood" varchar(100);--> statement-breakpoint
ALTER TABLE "travel_pulse_hidden_gems" ADD COLUMN IF NOT EXISTS "neighborhood" varchar(100);
