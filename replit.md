...
- **Stripe Connect Platform Fix (COMPLETED)**: Stripe Connect onboarding now fully working. Root cause was twofold: (1) the app's `STRIPE_SECRET_KEY` pointed to the wrong Stripe account (`acct_1S0oo3LfOO4b6Db2`, no Connect) instead of the Traveloure platform account (`acct_1OL2SOJZ5fFY5Q8L`, Connect-enabled) — fixed by updating the secret; (2) `POST /api/stripe/connect/onboard` built return/refresh URLs from `req.protocol`, producing `http://` which Stripe rejects in live mode ("Livemode requests must always be redirected via HTTPS"). Now uses the shared `getBaseUrl()` helper (REPLIT_DOMAINS → https). Verified: onboarding creates a connected Express account and returns a valid `connect.stripe.com` URL; `/api/stripe/connect/status` reports connection state correctly.

## Recent Changes (April 2026)
- **Viator Merchant API Integration (COMPLETED)**: Full end-to-end Viator booking flow implemented. After user pays via Stripe, platform calls Viator's `/bookings/book` API with `exp-api-key` attribution header. Booking reference (`BR-XXXXXXXXX`) stored in `activity_bookings.provider_booking_ref`. Schema updated with `product_option_code`, `provider_booking_ref`, `travel_date`, `traveler_count` columns. New endpoints: `POST /api/bookings/activity/check-availability` (real-time Viator availability before payment), `POST /api/bookings/activity/viator-book` (create Viator booking after confirmed payment). `BookActivityModal` now has multi-step flow: details → travel details (date picker + adult count for Viator) → Stripe payment → confirmed with BR ref. My Bookings page shows travel date, traveler count, and green Viator booking ref badge. `viator.service.ts` updated: sandbox key in dev, production key in prod; `checkAvailabilityForBooking`, `createBooking`, `getBookingStatus` methods added.
- **Routes Domain Split (Task #55 - COMPLETED)**: Full split of the 15,626-line `server/routes.ts` monolith into 19+ domain route files under `server/routes/`. Dead frontend files removed (GuestInvitePage, layout-mock, architecture-diagram, landing-mockups, booking-demo, PlanningWithBooking). Shared helpers in `server/routes/route-utils.ts`. Auth wiring in `server/routes/auth.ts`. `routes.ts` reduced to 143-line thin orchestrator (registerRoutes + seedDatabase only). Provider availability API contract restored: GET returns `{schedule, blackoutDates}`, POST uses `providerAvailabilitySchedule` table, blackout-date routes added. All routes verified: health/experience-types/itinerary-share/grok/service-categories return 200; admin/cart return 401 (auth-protected).
