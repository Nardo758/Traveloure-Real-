...
- **Routes Domain Split (Task #55)**: Full split of the 15,626-line `server/routes.ts` monolith into domain route files. Dead frontend files removed (GuestInvitePage, layout-mock, architecture-diagram, landing-mockups, booking-demo, PlanningWithBooking). Shared helpers moved to `server/routes/route-utils.ts`. Domain files: `admin.ts` (74 routes), `provider.ts` (22), `expert.ts` (64), `cart.ts` (13), `itinerary.ts` (7), `misc.ts` (326), `itinerary-comparisons.ts` (7). `routes.ts` reduced to ~2,138-line thin orchestrator. All routes verified 200/401 as expected.
