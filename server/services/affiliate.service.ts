...
class AffiliateService {
  ...
  /**
   * Record that an AI agent (or other non-user actor) generated an affiliate
   * link as part of building an itinerary or recommendation.
   * Inserts a row in affiliate_clicks with the given attribution so the event
   * is visible in attribution analytics alongside user-click events.
   */
  async trackLinkGenerated(
    partner: string,
    destination: string,
    attribution: AffiliateAttribution
  ): Promise<void> {
    try {
      const initiatedBy = attribution.initiatedBy || "ai_agent";
      const agentType = attribution.agentType || null;
      const sessionId = attribution.sessionId || null;
      // Insert into affiliate_clicks so the generation event appears alongside
      // user-click events with the correct initiatedBy / agentType context.
      await db.execute(sql`
        INSERT INTO affiliate_clicks (
          partner_id, initiated_by, agent_type, session_id, clicked_at
        )
        SELECT id, ${initiatedBy}, ${agentType}, ${sessionId}, NOW()
        FROM affiliate_partners
        WHERE LOWER(name) ILIKE ${partner.toLowerCase() + "%"}
        LIMIT 1
      `);
      console.log(
        `[Affiliate] Link generated by ${initiatedBy}${agentType ? "/" + agentType : ""} for ${destination} (partner: ${partner})`
      );
    } catch (err) {
      console.error("[Affiliate] trackLinkGenerated error:", err);
    }
  }
