...
class AIUsageService {
  ...
  async getTopUsersByPeriod(days: number = 7, limit: number = 20): Promise<Array<{
    userId: string;
    totalCalls: number;
    totalTokens: number;
    totalCostCents: number;
    ...
    const rows = await db
      .select({
        ...
        totalCostCents: sql<number>`sum(${aiUsageLogs.estimatedCostCents})::int`,
        lastUsed: sql<string>`max(${aiUsageLogs.createdAt})::text`,
      })
      .from(aiUsageLogs)
      .where(and(gte(aiUsageLogs.createdAt, startDate), sql`${aiUsageLogs.userId} is not null`))
      .groupBy(aiUsageLogs.userId)
      .orderBy(sql`count(*) desc`)
      .limit(limit);

    const userIds = rows.map(r => r.userId!).filter(Boolean);
    const providerRows = userIds.length > 0
      ? await db
          .select({
            userId: aiUsageLogs.userId,
          ...
          .where(and(
            ...
            sql`${aiUsageLogs.userId} = any(${sql.raw(`array[${userIds.map(id => `'${id}'`).join(',')}]`)})`
          ))
      : [];
    ...
    for (const r of providerRows) {
      ...
      if (!providerMap.has(r.userId)) providerMap.set(r.userId, new Set());
      providerMap.get(r.userId)!.add(r.provider);
    }
    ...
    return rows.map(r => ({
      ...
      providers: Array.from(providerMap.get(r.userId ?? "") ?? []),
      lastUsed: r.lastUsed,
    }));
  }

  async getDailyPerUserUsage(userId: string, days: number = 30): Promise<DailyUsage[]> {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    ...
    for (const log of logs) {
      const date = log.createdAt?.toISOString().split('T')[0] ?? '';
      if (!dailyMap.has(date)) dailyMap.set(date, { date, calls: 0, tokens: 0, costCents: 0 });
      const day = dailyMap.get(date)!;
      day.calls++;
      day.tokens += log.totalTokens;
      day.costCents += log.estimatedCostCents;
