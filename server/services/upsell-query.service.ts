...
export async function gatherOfferingCandidates(opts: {
  ...
  /** RETIRED no-op (ledger 2026-09-03-expert-templates-consumer-sunset). Formerly added
  ...
  try {
    ...
    // Shape A (expert packages as engine candidates) RETIRED with the expert-template lane
    // (ledger 2026-09-03-expert-templates-consumer-sunset): a ranked candidate whose only
    // destination was /expert-templates/:id has nowhere to land. `includePackages` is kept
    // as an accepted no-op option so the three Discover callers need no coordinated change.
