...
class RecommendationService {
  ...
  // getRecommendedPackagesForUser RETIRED (ledger 2026-09-03-expert-templates-consumer-sunset).
  // It ranked approved+published `expert_templates` for a city and rode GET
  // /api/recommendations/user as an additive `packages` field. That lane has no detail page
  // or purchase path any more, so a recommendation could not be acted on. `ready_made_trips`
  // is the surviving store lane and is surfaced by its own feed, not from here.
