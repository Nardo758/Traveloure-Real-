...
router.get("/api/admin/qa/verify", isAuthenticated, async (req, res) => {
  try {
    ...
    try {
      ...
      // Only App.tsx with NODE_ENV is acceptable
    ...
    try {
      ...
      // Check partial unique indexes for bookings
      ...
      results.D1 = {
        ...
        detail: missing.length === 0
          ? "All 5 critical constraints present"
    ...
    try {
      ...
      results.P3 = {
        pass: missing.length === 0,
        detail: missing.length === 0
          ? "All 4 webhook event cases present"
    ...
    try {
      ...
      const stages = rows.map((r: any) => `${r.stage}(${r.cnt})`).join(", ");
      results.A1 = {
        pass: total > 0,
    ...
    try {
      ...
      if (!row) { results.T2 = { pass: false, detail: "No expert form found for expert@traveloure-test.com" }; }
        ...
        results.T2 = {
          pass,
          detail: `status=${row.status} stripe=${row.stripe_connect_status} identity=${row.identity_verification_status}`,
        };
      }
    ...
    try {
      ...
      if (!row) { results.T3 = { pass: false, detail: "No provider form found for provider@traveloure-test.com" }; }
        ...
        results.T3 = {
          pass,
          detail: `status=${row.status} business_verification=${row.business_verification_status}`,
        };
      }
