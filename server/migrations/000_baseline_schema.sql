...
-- Idempotent baseline DDL for all Drizzle-managed tables.
-- Runs first in the migration chain so migrations 001+ can safely
-- reference all tables on a fresh database (CI, new deploys).
-- On existing schemas every statement is a no-op.
--
-- Transforms applied to pg_dump --schema-only output:
--   CREATE TABLE/SEQUENCE/INDEX  -> IF NOT EXISTS
--   CREATE TYPE                  -> DO $$ EXCEPTION WHEN OTHERS THEN NULL
--   ALTER TABLE ADD CONSTRAINT   -> DO $$ EXCEPTION WHEN OTHERS THEN NULL
-- Tables excluded (managed externally):
--   schema_migrations (migration runner), sessions (express-session)
...
-- [baseline] IDX_session_expire excluded (sessions table managed by express-session)
