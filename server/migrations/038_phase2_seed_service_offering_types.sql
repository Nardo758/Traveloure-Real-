-- Master Integration Brief — Phase 2: seed service_offering_types per SEED_DATA §4.
--
-- ~65 provider offering rows + 12 market-scoped variants. Each row joins
-- service_categories via category_key (the brief's join key).
--
-- is_surprising = true marks the "I never knew" row per brief §7.
...
-- ─── SEGMENT / EVENT (childcare_family, event_coordinator, etc.) ─────────────
INSERT INTO service_offering_types
  (offering_type_key, category_key, display_name, tagline, is_surprising, sort_order)
VALUES
  -- childcare_family
...
-- ─── Market-scoped provider offerings (SEED_DATA §4 bottom) ──────────────────
INSERT INTO service_offering_types
