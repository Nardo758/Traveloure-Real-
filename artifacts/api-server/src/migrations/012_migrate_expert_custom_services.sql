...
--
-- Idempotency: all statements use IF NOT EXISTS / NOT EXISTS guards.
-- If the source tables have already been dropped (migration 013 ran first on
-- a newer environment), the DO $$ blocks skip gracefully — no error thrown.
...
--    Skipped if expert_custom_services no longer exists (already dropped by migration 013).
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'expert_custom_services'
  ) THEN
...
--    Skipped if expert_selected_services no longer exists (already dropped by migration 013).
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'expert_selected_services'
  ) THEN
    INSERT INTO provider_services (
      user_id,
      service_name,
