...
- **Tested: 320/599**; remaining: **279**.
- Admin: **145/150**; payments: **29/31**; user-data: **146/204**; other: **0/214**.
...
- Evidence state: **fresh**; manifest SHA-256: `3ea41ce7334d2243aeebcf2880f6364d7f5b29be2eaca57108d9be42c2ca6b3d`; run timestamp: 2026-09-24T18:40:06.444Z.
...
| DELETE /api/coordination-bookings/:id | other | session-self | server/routes.ts:10674 | Other-category endpoint is intentionally outside the strict tested set. |
| DELETE /api/coordination-states/:id | other | session-self | server/routes.ts:10572 | Other-category endpoint is intentionally outside the strict tested set. |
...
| DELETE /api/provider/availability/:id | user-data | session-self | server/routes.ts:10351 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/coordination-bookings/:id | other | session-self | server/routes.ts:10632 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id | other | session-self | server/routes.ts:10467 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id/status | other | session-self | server/routes.ts:10495 | Other-category endpoint is intentionally outside the strict tested set. |
...
| PATCH /api/expert/bookings/:id/status | user-data | session-self | server/routes.ts:7452 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/expert/services/:id/status | user-data | resource-owner | server/routes.ts:6058 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| PATCH /api/provider-application | other | session-self | server/routes.ts:2758 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/provider/availability/:id | user-data | session-self | server/routes.ts:10327 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| PATCH /api/provider/bookings/:id/status | user-data | session-self | server/routes.ts:7456 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/provider/services/:id | user-data | resource-owner | server/routes.ts:4154 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| PATCH /api/service-bookings/:id/document-checklist | other | session-self | server/routes.ts:7900 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/service-bookings/:id/visa-status | other | session-self | server/routes.ts:7835 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/cart | user-data | resource-owner | server/routes.ts:9149 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/cart/convert-to-itinerary | user-data | resource-owner | server/routes.ts:9461 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/cart/resolve-trip | user-data | resource-owner | server/routes.ts:8946 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/coordination-bookings/:id/confirm | other | session-self | server/routes.ts:10658 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states | other | resource-owner | server/routes.ts:10415 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states/:coordinationId/bookings | other | session-self | server/routes.ts:10601 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/expert-application | other | session-self | server/routes.ts:2490 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/expert-booking-requests | other | resource-owner | server/routes.ts:1888 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/expert/:expertId/tip | payments | resource-owner | server/routes.ts:5662 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/expert/ai-tasks/:taskId/approve | user-data | session-self | server/routes.ts:11793 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/regenerate | user-data | session-self | server/routes.ts:11856 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/reject | user-data | session-self | server/routes.ts:11826 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/delegate | user-data | session-self | server/routes.ts:11694 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/expert/bookings/:id/complete | user-data | session-self | server/routes.ts:7623 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/reviews/:id/respond | user-data | session-self | server/routes.ts:8123 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/itinerary-comparisons | other | signature | server/routes.ts:9581 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/itinerary-comparisons/:id/generate | other | resource-owner | server/routes.ts:9923 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/provider-application | other | session-self | server/routes.ts:2719 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/provider-forms | other | session-self | server/routes.ts:2792 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/provider/availability | user-data | resource-owner | server/routes.ts:10299 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/provider/bookings/:id/complete | user-data | session-self | server/routes.ts:7622 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/provider/services | user-data | resource-owner | server/routes.ts:3822 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/provider/services/:id/deliverable-file | user-data | resource-owner | server/routes.ts:6420 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/recommendations/:id/convert | other | session-self | server/routes.ts:8602 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/:id/dismiss | other | session-self | server/routes.ts:8632 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/refresh/:city | other | session-self | server/routes.ts:8586 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/trips/:id/generate-itinerary | user-data | resource-owner | server/routes.ts:1593 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/vendors | other | session-self | server/routes.ts:2439 | Other-category endpoint is intentionally outside the strict tested set. |
...
| PUT /api/provider/services/:id/translations/:locale | user-data | resource-owner | server/routes.ts:3728 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
