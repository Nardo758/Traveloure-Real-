...
- Evidence state: **fresh**; manifest SHA-256: `d34912fe4998aeb52d8793b5031065395800d7cff622889d315c30d652cbee66`; run timestamp: 2026-09-24T18:11:40.531Z.
...
| DELETE /api/coordination-bookings/:id | other | session-self | server/routes.ts:10672 | Other-category endpoint is intentionally outside the strict tested set. |
| DELETE /api/coordination-states/:id | other | session-self | server/routes.ts:10570 | Other-category endpoint is intentionally outside the strict tested set. |
...
| DELETE /api/provider/availability/:id | user-data | session-self | server/routes.ts:10349 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/coordination-bookings/:id | other | session-self | server/routes.ts:10630 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id | other | session-self | server/routes.ts:10465 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id/status | other | session-self | server/routes.ts:10493 | Other-category endpoint is intentionally outside the strict tested set. |
...
| PATCH /api/provider/availability/:id | user-data | session-self | server/routes.ts:10325 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/cart | user-data | resource-owner | server/routes.ts:9147 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/cart/convert-to-itinerary | user-data | resource-owner | server/routes.ts:9459 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
| POST /api/cart/resolve-trip | user-data | resource-owner | server/routes.ts:8944 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/coordination-bookings/:id/confirm | other | session-self | server/routes.ts:10656 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states | other | resource-owner | server/routes.ts:10413 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states/:coordinationId/bookings | other | session-self | server/routes.ts:10599 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/expert/ai-tasks/:taskId/approve | user-data | session-self | server/routes.ts:11791 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/regenerate | user-data | session-self | server/routes.ts:11854 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/reject | user-data | session-self | server/routes.ts:11824 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/delegate | user-data | session-self | server/routes.ts:11692 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/expert/reviews/:id/respond | user-data | session-self | server/routes.ts:8121 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/itinerary-comparisons | other | signature | server/routes.ts:9579 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/itinerary-comparisons/:id/generate | other | resource-owner | server/routes.ts:9921 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/provider/availability | user-data | resource-owner | server/routes.ts:10297 | Resource-owner endpoint is not one of the 33 trip or two optimization real-fixture endpoints. |
...
| POST /api/recommendations/:id/convert | other | session-self | server/routes.ts:8600 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/:id/dismiss | other | session-self | server/routes.ts:8630 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/refresh/:city | other | session-self | server/routes.ts:8584 | Other-category endpoint is intentionally outside the strict tested set. |
