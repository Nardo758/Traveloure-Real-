...
- **Tested: 0/583**; remaining: **583**.
- Admin: **0/147**; payments: **0/31**; user-data: **0/196**; other: **0/209**.
...
- Evidence state: **evidence manifest SHA-256 is stale**; manifest SHA-256: `d4bac818f51d118e7095e695002d28516d09dfff3fb8872043acf47877d4614a`; run timestamp: 2026-08-26T18:45:07.509Z.
...
| DELETE /api/admin/notifications/:id | admin | admin-role | server/routes.ts:12687 | Not run: evidence manifest SHA-256 is stale. |
...
| DELETE /api/admin/slow-queries | admin | admin-role | server/routes.ts:11606 | Not run: evidence manifest SHA-256 is stale. |
...
| DELETE /api/cart | user-data | session-self | server/routes.ts:8990 | Not run: evidence manifest SHA-256 is stale. |
| DELETE /api/cart/:id | user-data | session-self | server/routes.ts:8972 | Not run: evidence manifest SHA-256 is stale. |
| DELETE /api/contracts/:id | user-data | session-self | server/routes.ts:11904 | Not run: evidence manifest SHA-256 is stale. |
...
| DELETE /api/coordination-bookings/:id | other | session-self | server/routes.ts:10239 | Other-category endpoint is intentionally outside the strict tested set. |
| DELETE /api/coordination-states/:id | other | session-self | server/routes.ts:10137 | Other-category endpoint is intentionally outside the strict tested set. |
...
| DELETE /api/provider/availability/:id | user-data | session-self | server/routes.ts:9916 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/admin/notifications/:id/read | admin | admin-role | server/routes.ts:12653 | Not run: evidence manifest SHA-256 is stale. |
| PATCH /api/admin/notifications/read-all | admin | admin-role | server/routes.ts:12713 | Not run: evidence manifest SHA-256 is stale. |
...
| PATCH /api/cart/:id | user-data | session-self | server/routes.ts:8900 | Not run: evidence manifest SHA-256 is stale. |
...
| PATCH /api/contracts/:id | user-data | session-self | server/routes.ts:11835 | Not run: evidence manifest SHA-256 is stale. |
...
| PATCH /api/coordination-bookings/:id | other | session-self | server/routes.ts:10197 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id | other | session-self | server/routes.ts:10032 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/coordination-states/:id/status | other | session-self | server/routes.ts:10060 | Other-category endpoint is intentionally outside the strict tested set. |
...
| PATCH /api/provider/availability/:id | user-data | session-self | server/routes.ts:9892 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| PATCH /api/service-bookings/:id/document-checklist | other | session-self | server/routes.ts:7519 | Other-category endpoint is intentionally outside the strict tested set. |
| PATCH /api/service-bookings/:id/visa-status | other | session-self | server/routes.ts:7454 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/bookings/:id/cancel | user-data | session-self | server/routes.ts:7569 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/cart | user-data | resource-owner | server/routes.ts:8714 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
| POST /api/cart/convert-to-itinerary | user-data | resource-owner | server/routes.ts:9026 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
| POST /api/cart/items | user-data | session-self | server/routes.ts:6730 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/cart/migrate | user-data | session-self | server/routes.ts:9002 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/cart/resolve-trip | user-data | resource-owner | server/routes.ts:8511 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
...
| POST /api/contracts/:id/communication | user-data | session-self | server/routes.ts:11887 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/contracts/:id/milestone | payments | resource-owner | server/routes.ts:11870 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
| POST /api/contracts/:id/payment | payments | resource-owner | server/routes.ts:11852 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
...
| POST /api/coordination-bookings/:id/confirm | other | session-self | server/routes.ts:10223 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states | other | resource-owner | server/routes.ts:9980 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states/:coordinationId/bookings | other | session-self | server/routes.ts:10166 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/coordination-states/:id/pay | payments | resource-owner | server/routes.ts:10293 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
| POST /api/coordination-states/:id/pay/confirm | payments | resource-owner | server/routes.ts:10501 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
| POST /api/coordination-states/:id/refund | payments | resource-owner | server/routes.ts:10577 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
...
| POST /api/expert/ai-tasks/:taskId/approve | user-data | session-self | server/routes.ts:11342 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/regenerate | user-data | session-self | server/routes.ts:11405 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/:taskId/reject | user-data | session-self | server/routes.ts:11375 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
| POST /api/expert/ai-tasks/delegate | user-data | session-self | server/routes.ts:11243 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/expert/bookings/:id/complete | user-data | session-self | server/routes.ts:7451 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/expert/reviews/:id/respond | user-data | session-self | server/routes.ts:7694 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/itinerary-comparisons | other | signature | server/routes.ts:9146 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/itinerary-comparisons/:id/generate | other | resource-owner | server/routes.ts:9488 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/provider/availability | user-data | resource-owner | server/routes.ts:9864 | Resource-owner endpoint is not one of the 32 trip or two optimization real-fixture endpoints. |
...
| POST /api/provider/bookings/:id/complete | user-data | session-self | server/routes.ts:7450 | Explicitly excluded in expert-provider-mutation-auth.test.ts; handler-owned real fixture is required before authorization can be claimed. |
...
| POST /api/recommendations/:id/convert | other | session-self | server/routes.ts:8173 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/:id/dismiss | other | session-self | server/routes.ts:8203 | Other-category endpoint is intentionally outside the strict tested set. |
| POST /api/recommendations/refresh/:city | other | session-self | server/routes.ts:8157 | Other-category endpoint is intentionally outside the strict tested set. |
...
| POST /api/trips/:tripId/activate-transport | user-data | session-self | server/routes.ts:12415 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/alerts | user-data | session-self | server/routes.ts:12633 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/budget/calculate-split | user-data | session-self | server/routes.ts:12026 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/contracts | user-data | session-self | server/routes.ts:11818 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/emergency-contacts | user-data | session-self | server/routes.ts:12568 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/emergency/initialize | user-data | session-self | server/routes.ts:12585 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/itinerary-items | user-data | resource-owner | server/routes.ts:12139 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/itinerary/optimize-order | user-data | resource-owner | server/routes.ts:12361 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/itinerary/reorder | user-data | resource-owner | server/routes.ts:12326 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/participants | user-data | resource-owner | server/routes.ts:11687 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/participants/bulk-invite | user-data | session-self | server/routes.ts:11723 | Not run: evidence manifest SHA-256 is stale. |
...
| POST /api/trips/:tripId/transactions | user-data | session-self | server/routes.ts:11983 | Not run: evidence manifest SHA-256 is stale. |
| POST /api/trips/:tripId/transactions/split | user-data | session-self | server/routes.ts:12000 | Not run: evidence manifest SHA-256 is stale. |
