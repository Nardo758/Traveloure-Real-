...
Generated from `server/routes.ts`. **596** raw mounted mutation registrations and **587** unique METHOD+normalizedPath pairs were found.

The unique-pair count is **+41** from the historical 546 comparison clue. The generator does not read that clue: it follows the current source mount graph. Raw registrations retain currently mounted, later-shadowed registrations; duplicate registrations are listed in the JSON manifest. A changed count indicates current route additions/removals or mount-graph changes, not an automatic regression.

Category totals: payments 31; admin 148; user-data 199; other 209.
Boundary totals: admin-role 148; session-self 305; resource-owner 90; signature 6; public-or-system 38; unknown 0.
...
| POST | `/api/itinerary-comparisons/:id/apply-to-cart` | other | session-self | no | unknown | `server/routes.ts:9980`<br>`server/routes/trips.routes.ts:822` | unknown | unknown |
...
| POST | `/api/itinerary-comparisons/:id/select` | other | session-self | no | unknown | `server/routes.ts:9950`<br>`server/routes/trips.routes.ts:793` | unknown | unknown |
| POST | `/api/itinerary-items/:id/backup` | other | session-self | no | unknown | `server/routes.ts:12516`<br>`server/routes/trips.routes.ts:1496` | unknown | unknown |
...
| POST | `/api/itinerary/estimate-travel` | other | session-self | no | unknown | `server/routes.ts:12610`<br>`server/routes/trips.routes.ts:1531` | unknown | unknown |
...
| POST | `/api/quick-start-itinerary` | other | session-self | no | unknown | `server/routes.ts:11265`<br>`server/routes/trips.routes.ts:862` | unknown | unknown |
