...
test("RV-1: refund reversal still records a negative compensation row after migration 203, and re-mint stays a no-op", async () => {
  const id = await insertBooking({ status: "completed", pi: PAID_PI, confirmedDaysAgo: 5, completedAt: new Date() });
  const booking = await storage.getServiceBooking(id);
  assert.ok(booking);
...
test("RV-2: negative compensation rows on BOTH earnings ledgers remain insertable after migration 203", async () => {
  const id = await insertBooking({ status: "completed", pi: PAID_PI, confirmedDaysAgo: 5, completedAt: new Date() });
  const booking = await storage.getServiceBooking(id);
  assert.ok(booking);
