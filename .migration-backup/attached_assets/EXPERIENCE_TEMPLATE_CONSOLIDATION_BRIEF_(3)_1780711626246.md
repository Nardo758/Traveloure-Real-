...
3. **Add tab-level filter-control support.** The Region-2 `flights` tab (max-price slider, stops toggle, sort) and `hotels`/`accommodations` tab (max price/night, stars, sort) are special-cased Amadeus search controls that are **NOT in `experience_template_tabs` today**. Represent them in the DB definition as tab-level control descriptors keyed by tab `type`, so retiring Region 2 (Phase 3) doesn't silently drop them.
   - *Out of scope:* `TemporalAnchorManager` and `ScheduleValidator` are **itinerary-page tools** (`my-itinerary.tsx`, `trip-details.tsx`), run *after* a trip is created — they are not experience-template controls and are not touched by this consolidation.
...
- **Spec alignment (`UNIFIED_PLANNING_FLOW_SPEC_v2`):** this brief changes only how the experience template *renders inputs* — it does not touch the entry-points → Cart → canonical Trip model, or the generate-(free)-vs-optimize-(paid) boundary. Guardrail: `contextField` values wire into the **free generate/draft + the trip/cart context**, NOT a backdoor free full-optimize — the paid optimize gate is unchanged. (Note: `TemporalAnchorManager`/`ScheduleValidator` are itinerary-page tools, not template controls — out of scope for this consolidation entirely.)
