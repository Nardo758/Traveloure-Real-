...
/**
 * PlanningModal - Captures user intent for trip planning
 * 
 * Usage:
 * <PlanningModal 
 *   isOpen={isOpen}
 *   onClose={() => setIsOpen(false)}
 *   initialDestination={{ city: "Paris", country: "France", cityId: "paris-fr" }}
...
export default function PlanningModal({ 
  ...
  onSuccess = null, // Callback when itinerary is generated
}) {
  // Form state
  ...
  const [step, setStep] = useState(1); // For multi-step flow if needed
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Call your existing AI API endpoint
      const response = await apiPost('/ai/generate-itinerary/', {
      ...
      // Store in sessionStorage for experience builder to pick up
      ...
      // Close modal and navigate or callback
      onClose();
      
      if (onSuccess) {
        onSuccess(data);
      } else {
        // Default: navigate to experience builder
        window.location.href = `/experiences/${experienceType}?tripId=${data.tripId}`;
      }
  ...
  // Add destination (for multi-city)
  const handleAddDestination = () => {
    if (!cityInput.trim()) return;
    
    // Simple parsing - in production, use a city autocomplete API
  ...
  // Calculate suggested trip length
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div>
            ...
            {/* Add destination input (for multi-city or if no initial destination) */}
