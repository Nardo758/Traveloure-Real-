...
test.describe("Marketplace cards — detail, storefront, and live-detail regressions", () => {
  ...
  test("/services keeps search state and exposes truthful service details with separate links", async ({ page }) => {
    ...
    // The storefront affordance is a sibling anchor in the current card design.
    // The preceding card design used a keyboard-operable role=link inside the
    // detail link instead; both shapes must navigate without introducing a<a>.
    const storefrontTag = await storefrontLink.evaluate((element) => element.tagName);
    if (storefrontTag === "A") {
      await expect(storefrontLink).toHaveAttribute("href", `/p/${service.providerHandle}`);
    } else {
      await expect(storefrontLink).toHaveAttribute("role", "link");
      await expect(storefrontLink).toHaveAttribute("tabindex", "0");
    }
    ...
    await expect(card).toContainText(/expert notes/i);
