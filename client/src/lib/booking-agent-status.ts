...
export interface BookingAgentStripCell {
  stage: BookingAgentStage;
  label: string;
  count: number;
}
...
/**
 * The strip: ONE cell per stage the rows ACTUALLY carry, in ruled order, with its count. Rows in
 * no stage produce no cell — the strip never draws an empty state for a stage nobody is in.
 * An empty input is an empty strip; the caller says "no requests" itself.
 */
const STRIP_ORDER: BookingAgentStage[] = [
  "received",
  "assigned_legacy",
  "researching",
  "ready_to_buy",
...
export function bookingAgentStrip(rows: readonly BookingAgentStatusRow[]): BookingAgentStripCell[] {
  const counts = new Map<BookingAgentStage, BookingAgentStripCell>();
  for (const row of rows) {
    const reading = readBookingAgentStatus(row);
    const cell = counts.get(reading.stage);
    if (cell) cell.count += 1;
    else counts.set(reading.stage, { stage: reading.stage, label: reading.label, count: 1 });
  }
  return STRIP_ORDER.filter((s) => counts.has(s)).map((s) => counts.get(s)!);
}
