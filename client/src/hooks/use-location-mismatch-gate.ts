 ...
 *
 * The decision lives in `client/src/lib/location-mismatch.ts`; this hook supplies it with the two
 ...
 *
 * NO NEW SERVER ROUTE, NO NEW DTO FIELD. Both facts are already on the wire:
 ...
 *   - the plan's destination — `GET /api/trips` already returns the traveler's own trip rows,
 *     and `client/src/components/add-to-experience-dialog.tsx` already reads it this way.
 *
...
/** The trip-identity slice of `GET /api/trips` this gate reads. Nothing else is touched. */
...
export function useLocationMismatchGate(targetTripId: string | null | undefined): LocationMismatchGate {
  ...
  // The same query key every other traveler surface uses, so this shares their cache rather than
  // adding a request. A 401 (guest) or a network failure leaves `data` undefined ⇒ silence.
  const { data: trips } = useQuery<GateTripRow[]>({
    queryKey: ["/api/trips"],
    enabled: tripId !== "",
  });

  const planDestination = useMemo<RawLocation>(() => {
    if (tripId === "" || !Array.isArray(trips)) return null;
    return trips.find((t) => t?.id === tripId)?.destination ?? null;
  }, [trips, tripId]);
