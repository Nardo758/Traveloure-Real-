/**
 * Provider Customers — Console IA C9 (§17 module 6, the provider nine-module stamp;
 * transplants the expert customers.tsx C4 pattern onto ProviderLayout).
 *
 * HONEST self-scoped aggregation: every row is derived from this earner's REAL transactions
 * via the same GET /api/me/customers aggregate the expert console uses (customers.routes.ts —
 * read-only, session-scoped §14). For a provider role the bookings lane (service_bookings
 * where providerId = session user) is the live source; the purchases and trips lanes are
 * structurally empty today (they hang off expert-authored ready-made listings and expert
 * trip assignments), but they stay rendered DATA-DRIVEN — a row only appears if a real row
...
function CustomerDetail({ customer }: { customer: CustomerRow }) {
  ...
  return (
    <div className="border-t border-console-light bg-console-bg/50 px-4 py-3 space-y-3">
      ...
      {/* Purchases/trips are expert-lane sources — structurally empty for a provider role
          today; rendered data-driven (plain rows, no cross-console links) if rows ever exist. */}
      {purchases.length > 0 && (
          ...
          <ul className="space-y-1">
            {purchases.map((p) => (
              <li key={p.id} className="flex items-center justify-between gap-3 text-sm">
                <span className="flex items-center gap-1.5 text-console-darkest min-w-0">
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            body="They appear with your first booking."
        ...
        <p className="text-xs text-console-mid">
          Booked value is what your customers paid across bookings — it is not your earnings.
          See Money for your ledger.
        </p>
      </div>
    </ProviderLayout>
  );
