...
export default function CreditsBillingPage() {
  ...
  const { data: balanceData, isLoading: balanceLoading } = useQuery<{ balance: number; total: number }>({
  ...
  const monthlySpent = transactions
    .filter(tx => {
      ...
      return tx.amount < 0 && isWithinInterval(d, { start: monthStart, end: monthEnd });
    })
    .reduce((sum, tx) => sum + Math.abs(tx.amount), 0);

  const maxMonthly = Math.max(monthlySpent, 100);
  const spentProgress = Math.round((monthlySpent / maxMonthly) * 100);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-gradient-to-br from-[#FF385C] to-[#E23350] text-white">
          <CardContent className="p-6">
            ...
            {balanceLoading ? (
              <Skeleton className="h-10 w-20 mt-1 bg-white/30" />
            ) : (
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <Progress value={spentProgress} className="mt-3 h-2" />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            {txLoading ? (
              ...
              <p className="text-3xl font-bold text-[#111827] dark:text-white mt-1" data-testid="text-bonus-credits">
                {transactions
                  .filter(tx => tx.transactionType === "bonus" && isWithinInterval(new Date(tx.createdAt), { start: monthStart, end: monthEnd }))
                  .reduce((sum, tx) => sum + Math.abs(tx.amount), 0)}
              </p>
            )}
        ...
        <TabsList className="bg-gray-100 dark:bg-gray-800">
          ...
          <TabsTrigger value="history" data-testid="tab-history">
            ...
            {transactions.length > 0 && (
              <Badge className="ml-2 bg-[#FF385C] text-white text-xs px-1.5 py-0">{transactions.length}</Badge>
            )}
          </TabsTrigger>
          ...
          <Card>
            ...
            <CardContent>
              {txLoading ? (
                ...
                <div className="text-center py-12" data-testid="tx-empty-state">
                  <InboxIcon className="w-12 h-12 mx-auto mb-3 text-gray-300" />
                  <p className="text-[#111827] dark:text-white font-medium">No transactions yet</p>
                  <p className="text-[#6B7280] text-sm mt-1">Your credit activity will appear here</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {transactions.map((tx) => {
                    const txType = tx.transactionType === "credit_purchase" ? "purchase"
                      : tx.amount > 0 && tx.transactionType === "bonus" ? "bonus"
                      : tx.amount > 0 ? "purchase"
                      : "spent";
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium text-[#111827] dark:text-white">
                              {tx.description || (txType === "purchase" ? "Credit Purchase" : txType === "bonus" ? "Bonus Credits" : "Credits Used")}
                            </p>
                            <p className="text-sm text-[#6B7280]">{format(date, "MMM d, yyyy")}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`font-bold ${tx.amount > 0 ? "text-green-600" : "text-[#111827] dark:text-white"}`}>
