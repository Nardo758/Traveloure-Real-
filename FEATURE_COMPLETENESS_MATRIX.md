...
| 7.13 | **Event coordination (Phase 4)** | 🟡 `DeliveryOptions.tsx:162` mentions event coordinator | 🟡 `routes.ts:7198+` coordination-states endpoints | ✅ `schema.ts:1598` `coordinationStates` + `eventCoordinationProfiles` | 🟡 `resolveCoordinationFee` (optimization-fee.service.ts:135) and `buildEventTimeline` (event-coordination.service.ts:86) exist but **not wired to any route** | 🟡 | `optimization-fee.service.ts:135` (orphan); `event-coordination.service.ts:86` (orphan); `routes.ts:7198` (endpoints) |
...
| **P0** | `resolveCoordinationFee` exists but **not wired to any route** | Event coordination (Journey 7) | 🟡 | `optimization-fee.service.ts:135` (service exists); **no route caller** | The coordination fee resolver ($499 floor / 8% of budget) is fully implemented and tested but no Express route calls it. The `/api/coordination-states` endpoints exist but do not call `resolveCoordinationFee`. |
...
| **P1** | Event optimize **endpoint missing** | Event optimize (Journey 7) | ❌ | `schema.ts:969` has event fee rows; **no event optimize endpoint** | No dedicated API for event optimization. Event coordination uses `resolveCoordinationFee` but that too is unwired. |
...
| 7. Event Coordination (Wedding/etc.) | 5 | 1 | 2 | 2 | 0 | **Event coordinator UI** (P1), **resolveCoordinationFee unwired** (P0), **buildEventTimeline unwired** (P0) |
...
2. **Two Phase 4 services are fully implemented but unwired**: `resolveCoordinationFee` (optimization-fee.service.ts:135) and `buildEventTimeline` (event-coordination.service.ts:86) have complete implementations and test suites, but no Express route calls them. These are the "built but never plugged in" class of gap.
