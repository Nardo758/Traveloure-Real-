 ...
 * is about — a storefront handle, a public service, or a booking they are already on — and the
 * SERVER resolves the counterpart from that. This is §14's identity rule (the actor comes from the
...
export const CONTACT_ADDRESS_KINDS = ["handle", "serviceId", "bookingId"] as const;
  ...
  .superRefine((value, ctx) => {
    ...
    if (given.length === 0) {
      ctx.addIssue({
        ...
        message: "Exactly one of handle, serviceId or bookingId is required",
      });
      return;
    }
    if (given.length > 1) {
