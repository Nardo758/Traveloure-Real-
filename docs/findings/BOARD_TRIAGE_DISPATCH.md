...
## Batch D — Operator / production. Ten, and only Replit can do them.
...
| **WEBHOOKS** | **STAGE 1 DONE 2026-09-22; STAGE 2 OPEN.** Platform endpoint `we_1UIbtDJZ5fFY5Q8LhomT6nVq` is live at `/api/bookings/webhooks/stripe` with exactly four types (`checkout.session.completed` + the three `customer.subscription.*`), proven end to end by a real live Checkout — `evt_1UIcxLJZ5fFY5Q8LhHkh4pIa`, 200 in 53 ms, one `webhook_events` row, zero unintended writes, charge refunded. Damage query came back clean: of 80 live Checkout Sessions all time, ZERO paid `expert_service` or `transport_booking` — it was a trap, not damage. **STILL DARK:** `payment_intent.*`, `charge.refunded`, `charge.dispute.*`, `transfer.*` — so §15c's recovery layer 2 still does not exist. Stage 2 is its own operator decision; the ownership map (succeeded → Connect ALONE, which is a superset; payment_failed → BOTH, they are complementary) is in the brief. Brief: `docs/briefs/STRIPE_WEBHOOK_DELIVERY_GAP.md`. |
...
**§20 applies to all ten.** The ONLY approvable publish prompt is an `ADD COLUMN IF NOT EXISTS`
matching a registered, unstamped migration. None of these ten adds a column — four are
