...
| **WEBHOOKS** | **HIGHEST-SEVERITY OPERATOR ITEM ON THIS LIST — read-only first, then a subscription change that is the decision-maker's.** Live Stripe has ONE enabled endpoint (`/api/webhooks/stripe`, Connect) subscribed to `account.updated` and NOTHING else, and NO endpoint at all points at `/api/bookings/webhooks/stripe`. The two handlers carry 14 event types between them; production delivers one. Two hosted-checkout rails (`transport_booking`, `expert_service`) complete ONLY through `checkout.session.completed` and have no fallback — and the expert one INSERTs its `expert_requests` row, so a paid session leaves no artifact at all. Step 1 is the read-only Stripe query that sizes it (see the brief). Brief: `docs/briefs/STRIPE_WEBHOOK_DELIVERY_GAP.md`. **The query writes nothing; the subscription change is a separate, ratified step.** |
