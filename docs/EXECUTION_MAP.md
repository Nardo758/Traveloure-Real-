...
| L17 | **`TrendingCities` per-city numbers** (Pulse scores, trending %, gems, traveler counts) — flagged by L8 as possibly fabricated, NOT verified either way | Haiku audit → fix | none | Distinct from the CityCard family's real `city.totalHiddenGems`; needs a source check before touching |
...
| L16 | **Real social proof + per-experience-type stats** — after L8 phase 1 strips the landing-page fabrications, the page has no social proof and categories have no stats. Needs: a curated testimonial feed (which real `service_reviews` get featured is EDITORIAL) and/or per-type aggregates (trending, rate ranges, gem/active counts) | Opus | **DECISION-MAKER**: is featured-review curation admin-approved? which stats are worth aggregating? | Do NOT invent either source; honest absence is correct until ratified |
