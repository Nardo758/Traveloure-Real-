...
and 3 route stops on "Gion Walking Tour with Photographer"). Screenshots under
...
| B7 | PASS | PATCH price (151.00 → 152.00) → 200, applied immediately in the response record, no `editReview`, stays Live. PATCH name → 200 with `editReview: { status:'pending', stagedKeys:['serviceName'] }`, live name unchanged, Catalog row shows Live + "Edit in review" with no "(rev)" in the displayed name. Raw request/response JSON preserved in `B7-patch-evidence.json`. Panel columns agree with observed behavior — no P1. (Staged name edit deliberately left pending as B7 residue; no discard control exists — follow-up filed.) | `B7-edit-split-behavior.png`, `B7-patch-evidence.json` |
