...
| B7 | PASS | PATCH price `{price:'151.00'}` → 200, applied immediately, no `editReview`, stays Live. PATCH name → 200 with `editReview: { status:'pending', stagedKeys:['serviceName'] }`, live name unchanged, row shows Live + "Edit in review". Panel columns agree with observed behavior — no P1 | `B7-edit-split-behavior.png` |
