...
| Server | `PORT=5001`, `NODE_ENV=development`, `OBJECT_STORAGE_DRIVER=memory`, `RATE_LIMIT_LOOPBACK_SKIP=1`, `STRIPE_SECRET_KEY=[REDACTED_STRIPE_TEST_KEY]`. Boot applied **208/208 migrations** (`[Migrations] Done — 208 newly applied`). |
...
by Stripe Identity and Stripe Connect. On this bench `STRIPE_SECRET_KEY=[REDACTED_STRIPE_TEST_KEY]`, so the real UI path is
