...
| 2 | Booking Concierge facilitation | **+5%** of item (band `expert_concierge_booking`, 064→066; the $9.99 flat is dead) — **superseded@7f14e21 (CLAUDE.md Locked Decision 51, ledger `2026-09-18-concierge-fee-cap-split`):** now capped at the band's own `max_amount` ($40.00, migration 311) and disclosed on `/pricing`; the expert's completion-time share is admin-editable via the new `expert_concierge_booking_expert_share` band (0.75 default) | added to traveler total, **now capped** | ✅ folded into `booking_commission`; expert share re-split at completion |
