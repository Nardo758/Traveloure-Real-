...
> **SUPERSEDED 2026-09-11 — the one decision register is `docs/PUNCHLIST.md` §1.** The rows below
> are kept as the record of what this program asked and how it was answered; they are NOT the place
> to add a new question — a new one goes in the punchlist. Every row here is now answered: A1/A2/A4
> below, and **A3 by the two fee rulings** (ledger `2026-09-08-cart-fee-line` and
> `2026-09-08-legacy-rail-fee`, both recorded as "register A3, accepted"). Two registers guarantee
> one gets answered and the other quietly does not.
...
| ~~A3~~ | ✅ **ANSWERED (2026-09-08).** The decision-maker accepted the correction and it landed on BOTH rails — ledger `2026-09-08-cart-fee-line` (cart/`service_bookings`) and `2026-09-08-legacy-rail-fee` (`process-cart`/`bookings`). The provider's commission is a deduction from the payout and is never a line added to the buyer; the traveler service fee is resolved from `fee_bands` (§8) and composed server-side (§14) by the single `composeTravelerCharge`. Original text: ⏳ **DEFAULT ACCEPTED, NUMBERS TO BE RE-CONFIRMED AT THE CHARGE (2026-09-07).** The ruling taken: the cart shows the TRAVELER service fee (7% capped at $25) resolved from `fee_bands`, and the PROVIDER commission is never added on top of the traveler's price. Because this moves money, the cart lane must put the resolved figures in front of the decision-maker BEFORE the line changes — §14 (server-derived amount) and §8 (no rate literal) bind the implementation either way. **No checkout ships before that confirmation.** | Cart lane |
