...
| ~~Optimizer (until lane 6)~~ | — | — | READS (via projection) | — | **SUPERSEDED by the row below (Lane 5b, Jul 31 2026).** Historical: saw only the projected cart; the behavior-identical merge gate held until the re-point landed — it is now retired |
| **Optimizer (Lane 5b)** | READS | **NEVER** | READS | READS (**constraint only**) | **Finalized + decision-maker RATIFIED Jul 31, 2026** (label corrected from the placeholder "after lane 6"). Optimizable set = `in_planning` + `ready_for_checkout` — both are still plan, so both are the optimizer's to re-plan. `purchased` is read **as an anchor-style fixed constraint only**: injected into the generation prompt as an immovable commitment the plan is built around, **never re-plannable, never emitted in a variant** (enforced server-side, not merely asked of the model), never deleted at apply. `with_expert` is **NEVER** read — expert-owned and still fluid: not a constraint, not an input. Writes no routing status, ever |
