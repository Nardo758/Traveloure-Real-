...
for (const { r, g } of items) {
  ...
  md += `| ${g.severity} | ${strike}${g.class}${strike} | \`${esc(r.id)}\` | ${esc(r.route)} | ${esc((r.label?.text || "").slice(0, 50))} | ${r.verdict.evidence === "behavioral" ? `**${esc(r.verdict.evidenceRef)}**` : `static \`${esc(r.trigger?.loc)}\``} | ${ui} | ${strike}${esc(g.note || "")}${strike}${g.uiObserved ? ` — _U2: ${esc(g.uiObserved.slice(0, 160))}_` : ""} |\n`;
}
