...
| V-7 | ~~**Any signed-in account can stamp any transport option `confirmed`, with any reference.**~~ **CLOSED 2026-09-12 (ledger `2026-09-12-transport-status-self-report-gate`).** The diagnosis stood exactly as written. `PATCH /api/transport-booking-options/:optionId/status` now: resolves the option, derives its owning scope (`variant_id`, or the leg's variant → comparison) and authorizes it through the SAME `authorizeTransportScope` the hub read uses, with `requireWriteAccess` since a self-report is a mutation (LD 12); parses a **pick-based `.strict()` allowlist** whose value set is `available \| booked \| cancelled` — **`confirmed` is not in it**, because that value is the payment's word and a traveller may not type themselves into a machine state (LD 44 (e)); answers **ONE 404** for an absent row, an unownable row and a refused caller alike, so the rail cannot be used to probe which options exist (LD 40's `conversations/start` posture); **refuses a PLATFORM option outright**, since its status follows the money and a hand-written `booked` would reopen V-1 through a second door; and writes with a from-state guard IN the statement so a self-report can never overwrite a payment-written `confirmed`. The reply reports the PERSISTED row, never an echo. Pinned by `confirm-honesty.test.ts` C1–C5 (structural, no DB — stated in that file's negative space). | closed |
