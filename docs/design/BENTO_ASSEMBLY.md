...
1. §2 — CLOSED (anchor priority landed 2026-08-26, `db59aafd`). Re-verified 2026-08-27 directly against real Mumbai data: Rhea Desai (`role: event_planner`) is correctly excluded from anchor selection; the exclusion check already matched the real underscore-separated role value. `bento-anchor.ts`'s neighbourhood-token normalizer was additionally hardened to mirror `normalizeNeighborhoodKey` (collapse any punctuation run, not just whitespace/underscore) so a punctuated real neighbourhood name (e.g. "Fort / Kala Ghoda") is no longer only matched by coincidence — see 2026-08-27-neighbourhood-slug-match in DECISIONS.md.
...
3. §5 — CLOSED 2026-08-27. Gem tiles were showing the bare score (`85`) with no label in the compact bento meta line. Fixed by a new `gemScoreMetaFragment()` helper (`client/src/lib/gem-score.ts`) that always renders `gem score {N}` and returns `null` (fragment omitted, joined by `joinMeta`) on zero/null/invalid — never a bare label or a bare number.
