...
Step2Where (formerly `ModalWhere`), occasion "Golf trip", but the field is now an **ordered stop list**: 01 Edinburgh,
...
1. A golf trip needs an **ordered, numbered, multi-stop** list — same UI shell as Step2Where, but
...
- The ordered-stops gap is ruled HELD (same citation as the Step2Where brief).
