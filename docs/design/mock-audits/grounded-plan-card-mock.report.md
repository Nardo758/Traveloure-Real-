...
| MATCH | The affiliate rung only links confidently matched rows with a ratified bookable classification (`affiliate_bookable` or `in_platform_bookable`); unclassified rows fall through. | `server/services/slip-grounding.service.ts`; `server/services/affiliate-grounding.service.ts`; `server/services/slip-grounding-match.ts` |
...
The affiliate grounding eligibility gate is now enforced before persisting an affiliate link; unclassified items fall through to DMO or remain honest suggestions. Focused server and client tests cover the fail-closed behavior.
