...
- `2026-08-27-pricing-nav` — Pricing is a plain text link in the right-side utility cluster of the nav (`Ways to Earn · Pricing · Sign In`), not a dropdown; same `/pricing` route as the footer link; the nav leaf and the page ship together in the `/pricing` build lane. `nav-config.ts`, one leaf, no icon (right cluster is text).
