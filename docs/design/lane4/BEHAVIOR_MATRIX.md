...
| `Book now` | gem/vendor → `/services/:id` (date/time → cart → audited `/api/checkout`); rec → `/services?categoryKey=` | gem `bookHref` `city-feed-card.tsx:739`; vendor `window.location=/services/:id` `:1515`; rec `onBook` → `handleBookRecommendation` `discover-location.tsx:1773` | `real-data` (needs a bookable service row) | ⏳ real-data |
...
| `Ways to earn` | Recruitment deep-link with `city` + `neighborhood` + `offering`; the landing hero uses the same vocabulary and links to `/earn` | `link-wanted-apply` `feed/wanted-slot-card.tsx`; `hero-wanted-cta` `landing/landing-hero.tsx` | `spec` — test 9 (deep-link shape) + `landing-hero-representative.test.tsx` | ✅ green |
