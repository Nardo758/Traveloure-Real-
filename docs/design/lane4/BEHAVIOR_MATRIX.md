...
| Recommendation impression | `POST /api/upsell/impression` fires once per rendered rec tile (on mount) | `useEffect` + `impressionFiredRef` `city-feed-card-recommendation.tsx:140` | `spec` — test 9 (rec tile renders) · POST-once count = `real-data` | ✅ renders / ⏳ POST-once |
