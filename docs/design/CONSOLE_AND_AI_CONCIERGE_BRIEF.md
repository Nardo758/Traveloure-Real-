...
| **L16-ask-ai-drawer** | ruling 3 | no | L15 · **its own brief, revised to v3: `docs/design/ASK_AI_DRAWER_BRIEF.md`** · decision rows **D-45..D-50** | The drawer, pre-final on the slip and post-final on the Trip Card. **CORRECTED by punchlist D-19 (ledger `2026-09-15-d19-plan-proposals`, migration 299): a task does NOT answer on the expert suggestions rail** — `trip_suggestions.expert_id` is NOT NULL and its approve path hardcodes `origin:'expert'`, the false attribution LD 42 D4/D23 forbid — it answers as a `plan_proposals` row. **The store, the discard, the pay and the apply are BUILT** (D-19; D-20/D-21, ledger `2026-09-15-d20-d21-proposal-charge`, migration 300: flat from the `concierge:ai_task` band, ONE charge per distinct proposal applied, §15b claim, expert items protected per D3). **What is MISSING is the CREATE rail and the drawer UI** — specified in the new brief. | `SlipRail.tsx` · `SlipView.tsx` · `PlanCard.tsx` rail · `server/routes/trips.routes.ts` proposal routes · `plan-proposals.service.ts` · `proposal-charge.service.ts` · `GET /api/pricing` `aiTaskCents` | check-money-endpoints · phase2-fee-gate · check-ai-draft-eligibility · the landed `plan-proposals` / `plan-proposal-charge` / `proposal-apply-authorization` suites |
...
| L16-ask-ai-drawer | **brief REVISED to v3** (`docs/design/ASK_AI_DRAWER_BRIEF.md` — the create rail and the drawer now specified); L15 landed; the store and charge point are BUILT. **Blocked on decision rows D-45..D-50** — the create rail needs D-45/D-46/D-47/D-50, the drawer's visibility and coverage line need D-48, and the post-final mount needs D-49. | `2026-09-15-l16-ask-ai-drawer-brief` |
