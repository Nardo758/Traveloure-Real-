...
- On dispute creation, mark affected bookings disputed, hold unpaid earnings, and prevent an unclaimed provider payout from being sent while the dispute is open. A pending payout can remain pending with an explicit hold reason and be reviewed after resolution; it must not silently transition to `processing`.
- A payout already in `processing` may have been sent to Stripe, even if the local transfer ID has not been recorded. Never automatically cancel, retry, or reverse it. Alert an operator for reconciliation. On a lost dispute, flag transferred earnings/payout IDs for a manual reversal decision; do not invoke a transfer reversal.
...
- Use event-ID deduplication for new platform processing with a separate per-consumer claim, not the shared `webhook_events.processed` flag: the existing platform and Connect handlers may both receive an event ID for different business work. A handled event must durably finish before HTTP 2xx; errors must be retryable. Do not change existing payment or subscription event ownership.
...
Proposals only, not part of this migration: move `childcare_family` from risk `moderate`/band `2` to risk `high`/band `3`; choose a conservative reviewed default for `custom_other` rather than silently accepting a null risk/band. These require their own approval and pricing/risk impact review before implementation.
