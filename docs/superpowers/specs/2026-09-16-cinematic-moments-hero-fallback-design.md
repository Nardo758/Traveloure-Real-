...
## Hero card behavior
Remote API imagery remains first choice. The local-expert card uses one neutral bundled photo of a real guide only when the expert has no usable profile image; it never borrows Wedding, Proposal, or other Moment imagery. Gem and service cards retain their existing fallbacks. Missing or failed remote URLs switch to the local image rather than exposing the gradient. Fallback imagery carries the existing Reference photo label. The readability overlay remains above the image and below content and badges.
