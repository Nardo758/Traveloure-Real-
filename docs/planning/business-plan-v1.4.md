...
**As of:** `0b9aff8c5` (2026-09-24) — reconciled against `main` per ledger `2026-09-24-plan-v14-trip-pass-benefits`; see Appendix D for the change log.
...
| Trip Pass | unlimited runs and AI tasks on one plan; traveler service fee waived on that plan's bookings | $19 per trip | `plans.trip_pass` | Live |
...
**Trip Pass sells three benefits and enforces three.** `TripPassAction` is `optimizer_run`, `ai_task`, `traveler_service_fee` (`server/services/trip-entitlement.service.ts`). A fourth — one expert revision — was retired on September 21, 2026 (ledger `2026-09-21-expert-revision-retired`): the entitlement, its `consumeRevision` hook and the `allowances_snapshot.revisionsRemaining` write were deleted, not left dormant, because the expert-revision *product* already exists without them — a `plan_work` listing purchase grants the selling expert write access on the buyer's plan at the expert's own price (Locked Decision 11). What was refused was the Trip Pass *inclusion*, which would mean the platform paying an earner at a platform-set rate. Re-introducing it is a new money decision, not a re-wiring.

**Trip Pass is on sale now.** It is purchased on the slip (`TripPassCard` → `POST /api/trips/:tripId/trip-pass/purchase` → Stripe → `/purchase/confirm`), mounted behind no flag. The 2026-10-01 memberships lane (§11, Wave 0) adds its Stripe **Price** row alongside Plus's and Pro's; Trip Pass is charged by PaymentIntent, not by subscription, so that lane does not gate its sale. Plus and Pro are the two that are not yet sellable.
...
| 1 | Venue directories, per market: ingest the registered Tier 3 venue registries (National Trust UK, VenueMonk India, Kyoto temple venues) into places under the scrape-facts rule; every venue found becomes a recruitment lead for the `venue` provider category (migration 285) | §4 coverage table | none (registry rows exist; ingestion adapters are the work) |
| 2 | Venue directories, the other five markets: register a Tier 3 venue source per market (Bogotá, Cartagena, Goa, Jaipur, Porto) and run the same ingestion; where no registry exists, the market's launch checklist gains a "venues seeded" check and the recruitment blueprint carries the list | §4 coverage table, §8 | brief (per-market source registration) |
...
|---|---|---|
| 2026-09-24 | Reconciled against `main` @ `0b9aff8c5`, §3 only. **§3.1 Trip Pass no longer lists *one expert revision*** — retired in code on 2026-09-21 (ledger `2026-09-21-expert-revision-retired`; the `expert_revision` `TripPassAction`, `consumeRevision` and the `revisionsRemaining` snapshot write were deleted, and the two traveler-facing surfaces stopped advertising it two days after this doc's previous as-of). The pass sells and enforces three benefits: `optimizer_run`, `ai_task`, `traveler_service_fee`. A paragraph under the table names them and records why the fourth was a deletion and not a build. **§3.1 Trip Pass Status corrected to Live** — it was *"Built; checkout lane starts 2026-10-01"*, which is the memberships lane's date; Trip Pass is charged by PaymentIntent, not by subscription, and sells on the slip today behind no flag. A second paragraph says what October adds for it and that Plus and Pro are the two not yet sellable. **Checked and left alone, with reasons in the ledger row:** §3.4's three non-existent row keys (copied faithfully from `PRICING_AND_FEATURE_MAP.md`, which is the authority §3 cites and where the drift originates — five documents carry them; the printed values are correct), and §3.2/§3.3's largely unenforced Plus and Pro benefit lists (neither product is on sale, so they are plans, not false promises). The ruling `2026-09-24-ai-first-draft-waiver` is not on `main` and is unimplemented, so §3's `$2.99` per AI task still describes what the code charges. No figure moved; the model script was rerun unchanged. | `2026-09-24-plan-v14-trip-pass-benefits` |
