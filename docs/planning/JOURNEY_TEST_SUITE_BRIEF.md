...
<!-- superseded@9382d500 — the original J12 text (dual-rate attribution → rails band) is INVALIDATED by ruling 45: the mechanism it asserted does not exist in code and the journey could never pass. Rewritten rather than annotated because the ruling explicitly ordered the rewrite; the superseded claim is preserved verbatim in docs/testing/PROVIDER_SIGMA_AUDIT.md MI-4 and in ruling 45. -->
**J12 — Attribution vs. rate: the SINGLE-BAND divergence pin (money-critical).** *Rewritten by ruling 45 (provider-sigma Q4). The previous text asserted "commission resolves to rails band via fee_bands" and **could not pass**: there is no `rails` band in `fee_bands`, `decideBandKey` accepts no attribution input, and `payments.routes.ts:575-577` explicitly declares acquisition attribution an "Analytics dimension only: never read into any fee/amount/payout decision". The dual-rate model is spec-ahead-of-code in its ENTIRETY — the gravity audit recorded only the repeat-pair rate as unbuilt; the whole mechanism is. Same correction ruling 38 made to J1/J2's payment leg: pin the truth, do not encode a mechanism that does not exist.*

- **J12.1 (builds now — the divergence pin, R22 shape).** Provider generates an attributed short link → traveler A books through it → assert the `service_bookings.acquisitionRef` → `short_links.code` chain is intact and the attribution REPORTING query returns the booking → traveler B books the same service platform-sourced → **assert the two resolved commission rates are IDENTICAL**, and that the single rate equals the `fee_bands` row read from the DB (no literal anywhere in the assertion). Green because the divergence is present; **it FAILS LOUDLY the day a rails/attribution band is seeded**, which is exactly when J12.2 becomes buildable.
- **J12.2 (`deferred:provider-backoffice-p1`).** The dual-rate steps — attributed → rails band, platform-sourced → full band, the two rates differ, both from `fee_bands`. **The headline requirement transfers to provider back-office Phase 1**, which owns BUILDING the mechanism; this journey only proves it once it exists.
- **Fixture note (ruling 44):** the attributed short-link fixture is scoped DOWN by this rewrite — with no rate consumer, its only consumer is attribution reporting (`short-links.routes.ts:213-228`).
