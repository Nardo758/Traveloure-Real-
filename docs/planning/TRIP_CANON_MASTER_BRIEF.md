...
| Optimizer is cart-based (spec said tripId) — W2 behavior-identical constraint | Audit P1 / Scope §0 | **L5b — DONE** | **Ratified + landed Jul 31, 2026.** Baseline = the trip's own `itinerary_items`: `in_planning` + `ready_for_checkout` optimizable, `purchased` read as anchor-style **constraints only** (in the prompt, never emitted in a variant — enforced server-side), `with_expert` never read. Contract row finalized (`ROUTING_STATE_CONTRACT.md` §2, "Optimizer (Lane 5b)"); W2 merge gate retired. Apply-to-trip dedupes proposed items against surviving routed rows so a re-applied plan never duplicates a `ready_for_checkout`/`purchased` item |
...
| **G2 guest trips** (session Trip / migrate-on-signup reshape) · Guest expiry unowned · L7 afterlife (rebooking, RM prompts) · repeat-pair rails rate (spec-ahead) · guest-invite trip linkage (#154) · no RM review object | Audit P3 | **deferred** | Documented inventory; pulled only explicitly. **Carries a retirement obligation from L5b (ratified Jul 31, 2026): the optimizer's guest-only cart-read fallback retires when G2 ships guest trips.** It is deliberate debt — the branch is labelled guest-only and is unreachable today (`isAuthenticated`), and a signed-in caller with an empty trip gets `409 trip_empty_convert_cart` instead. When G2 lands, delete both cart-read branches in `server/routes.ts` (create + `/:id/generate`) so the Trip is the single optimizer baseline for every principal |
