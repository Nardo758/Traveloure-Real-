...
| Rail | Where | Shape |
|---|---|---|
| Candidates read | `GET /api/itinerary-comparisons/:id/anchor-candidates` — `server/routes/trips.routes.ts:688` (owner-gated) | `RankedAnchors { hotel: AnchorScore[]; neighborhood: AnchorScore[]; activity: AnchorScore[] }` — `anchor-candidates.ts:31` |
