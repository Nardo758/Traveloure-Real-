...
3. **Add tab-level filter-control support.** The Region-2 `flights` tab (max-price slider, stops toggle, sort) and `hotels`/`accommodations` tab (max price/night, stars, sort) are special-cased controls that are **NOT in `experience_template_tabs` today**. Represent them in the DB definition as tab-level filter controls so retiring Region 2 (Phase 3) doesn't silently drop them. Either extend `experience_template_tabs` with a tab-level control descriptor, or model flights/hotels as a known control set the engine renders by tab type.
