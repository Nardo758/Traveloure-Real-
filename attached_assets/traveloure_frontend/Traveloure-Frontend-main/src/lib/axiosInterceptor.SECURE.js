  ...
  async (error) => {
    ...
    if (error.response?.data) {
      ...
      if (isTokenExpired(errorData)) {
        logger.debug('🔒 Token expired detected in axios response:', errorData)
      ...
      if (refreshToken) {
        try {
          ...
          if (response.data?.access) {
            ...
            logger.debug('🔒 No access token in refresh response - logging out')
            handleTokenExpiration()
          ...
          if (refreshError.response?.data && isTokenExpired(refreshError.response.data)) {
            logger.debug('🔒 Refresh token also expired - logging out')
          }
  ...
  customAxios.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response?.data && isTokenExpired(error.response.data)) {
        logger.debug('🔒 Token expired detected in custom axios response:', error.response.data)
        handleTokenExpiration()
        return Promise.reject(new Error('Token expired'))
      }
