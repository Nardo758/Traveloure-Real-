...
export default function ServiceProviderStatusPage() {
  const { data: session } = useSession();
  const { serviceProviderStatus, getStatus, loading } = useServiceProvider();
  ...
  const statusRaw = serviceProviderStatus?.data?.status;
  const status = typeof statusRaw === "string" ? statusRaw.toLowerCase() : "";

  // For service provider, use business_name, name, email from the API
  const name = serviceProviderStatus?.data?.name || session?.user?.name || "User";
  const email = serviceProviderStatus?.data?.email || session?.user?.email || "";
  ...
  if (status === "pending") {
    ...
    message = `You haven't submitted a service provider application yet. Please complete the registration to get started.`;
  }
