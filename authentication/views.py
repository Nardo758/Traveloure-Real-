...
import uuid
from .utils import generate_tokens,CustomPagination
...
class UserLoginAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({"message":"Login Successfull","status":True,"tokens": tokens}, status=200)
...
class SearchLocalExertAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        experts = User.objects.using('default').filter(
            Q(is_local_expert=True) &
...
class LocalExpertCreate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'message':'Your Request for Becoming a Local Expert is submitted Successfully. You will be notified once you are verified. Stay Still','status':True},status=200)
...
class LocalExpertAdminDetailUpdateAPIView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request,*args,**kwargs):
        queryset = self.get_object()
        serializer = LocalExpertRegisterationStatusSerializer(queryset,partial=True,data=request.data)
        if serializer.is_valid():
            serializer.save()
...
class ManageServiceProviderFormDetailUpdateView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        queryset = self.get_object()
        serializer = AdminServiceProviderUpdateStatusSerializer(queryset,partial=True,data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({'message':"Status Updated Successfully",'status':True},status=200)
        return Response({'message':f"Error Occured: {serializer.errors}","status":False},status=400)
