...
---
The workspace plan map is a build-time branch (`MAPS_KEY ? Google : Leaflet`) PLUS a runtime detector: Google calls `window.gm_authFailure` on key rejection (RefererNotAllowedMapError, expired key, billing), which flips all Google map surfaces to their fallbacks.
